// Taken from https://github.com/wacki/Unity-IndentShader
Shader "IndentSurface/DentableSurface" {
	...
	SubShader {
        ...
        {
            const float3 off = float3(-0.1f, 0, 0.1f); // texture resolution to sample exact texels
            const float2 size = float2(0.01, 0.0); // size of a single texel in relation to world units

            float s01 = tex2Dlod(_Indentmap, float4(texcoord.xy + off.xy, 0, 0)).z * _IndentDepth;
            float s21 = tex2Dlod(_Indentmap, float4(texcoord.xy + off.zy, 0, 0)).z * _IndentDepth;
            float s10 = tex2Dlod(_Indentmap, float4(texcoord.xy + off.yx, 0, 0)).z * _IndentDepth;
            float s12 = tex2Dlod(_Indentmap, float4(texcoord.xy + off.yz, 0, 0)).z * _IndentDepth;

            float3 va = float3(size.xy, s21 - s01);
            float3 vb = float3(size.yx, s01 - s21);
        ...
        void vert(inout appdata v) {
            //v.vertex. += sin(_Time.y * 1.0f + v.vertex.y * 1.0f) * 1.0f;
            float height = tex2Dlod(_Indentmap, float4(v.texcoord.xy, 0, 0));
            v.vertex.y += height * _IndentDepth;
            v.normal = normalize(calcNormal(v.texcoord));
        }
